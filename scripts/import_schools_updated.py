...
# 导入Django模型
from recommendation.models import School
        ...
        for _, row in df.iterrows():
            try:
                ...
                if '所在省份' in df.columns and pd.notna(row.get('所在省份')):
                    school.province = str(row.get('所在省份')).strip()
                
                if '所在城市' in df.columns and pd.notna(row.get('所在城市')):
                    school.city = str(row.get('所在城市')).strip()
                
                if '学校类型' in df.columns and pd.notna(row.get('学校类型')):
                    school.type = str(row.get('学校类型')).strip()
                
                if '学校等级' in df.columns and pd.notna(row.get('学校等级')):
                    school.level = str(row.get('学校等级')).strip()
                ...
                for excel_field, db_field in fields_mapping.items():
                    if excel_field in df.columns and pd.notna(row.get(excel_field)):
                        ...
                        # 特殊处理布尔类型
                        if db_field in ['is_985', 'is_211', 'is_double_first_class']:
...
def main():
    """主函数"""
    if len(sys.argv) < 2:
        print("用法: python import_schools_updated.py <Excel文件路径>")
        return
